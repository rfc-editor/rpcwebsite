...
# data
# --------------------------------------------------------------------------- #
function get_weeks_in_state($pdo,$state,$internal_key,$draft_name){
$debug_shl = false;
$weeks_in_state = "";
#$data_array = get_weeks_state_array('3658');
$data_array = get_weeks_state_array($pdo,$internal_key,$debug_shl);
...
function get_weeks_state_array($pdo,$internal_key,$debug_shl){
 
   $sql = "SELECT st.`state_name` , sh.`iana_flag` , sh.`ref_flag` , sh.`in_date` , sh.`version_number`, sh.`generation_number` FROM states st, state_history sh WHERE st.state_id = sh.state_id AND sh.internal_dockey = :internal_key  ORDER BY in_date";
