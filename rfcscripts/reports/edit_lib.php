...
# $Id: edit_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: edit_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: edit_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: edit_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: edit_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
...
#     array('internal_key', 'draft', 'state', 'iana', 'date_received', 'expiration', 
	...
	'state_id','generation_number','consensus_bit','rfc_editor','xml_file');
...
function pull_from_db($key, $raw=0, $keyfield="draft") {
    ...
    #Open DB needs to be called before mysqli_escape_string
    $db_link = open_db();
    ...
    if ($keyfield == 'internal_key') {
         $query .= " = $key";   // we're matching by id number
    } else {
         $query .= "regexp \".*($key).*\""; // we're comparing strings
    }

//    $query .= " AND draft LIKE CONCAT(draft_base,'-__')";
    ...
    if ( !($result = mysqli_query($db_link,$query)) ) {
         error_log( "Bad query: $query  - " .mysqli_error($db_link) );
         return array('error' => "Bad query: $query  - " .mysqli_error($db_link) );
    }
    if ( ($rows = mysqli_num_rows($result)) != 1 ) {
	if ( $rows < 1 ) return array('error' => "No such draft: $key");
	else 
	    return array('error' =>
...
function matching_links($key, $keyfield="draft", $page="edit_draft.php") {
   ...
   # open_db();
    ...
    if ( !($result = mysqli_query($db_link,$query)) ) 
	return array("Bad query: " . htmlspecialchars($query) . " - " .
...
function table_text_input($name, $width=70) {
    ...
    <tr>
	<td>
           <label $label_class for="$name">$field_labels[$name]:</label>
        </td>
	<td>
           <input type="text" $attr size="$width" name="$name"
	    value="$fields[$name]" id="$name"/>
        </td>
    </tr>
END;
}
...
function table_textarea($name, $rows=1, $cols=80) {
    ...
    if ( $rows ) $dim = " rows=\"$rows\"";
    if ( $cols ) $dim .= " cols=\"$cols\"";
    print <<<END
	<tr>
	    <td>
		<label $label_class for="$name">$field_labels[$name]:</label>
	    ...
	    <td>
		<textarea $dim $attr name="$name" id="$name">$fields[$name]</textarea>
	    </td>
	</tr>
END;
}
...
function hidden_form($dest, $buttons) {
    ...
    print "<form action=\"$dest\" method=\"post\">\n";
	...
	print "<input name=\"submit\" " .  "value=\"$button\" " . 
	    "type=\"submit\"/>\n";
...
function edit_document_form($dest) {
    global $fields;
    global $wgs,$statuses, $yes_no, $states, $types, $consensus_bit;
    global $editor_field_names;
    global $debug_edit_lib;
...
END;
    ...
    table_checkbox_input("xml_file");

#TODO need to change the array from state lookup
    #Get the state names from the database

#    table_select("state", $states);
...
function check_fields() {
    ...
    foreach ($refs as $ref){
        $ref_cid = get_cluster_id(trim($ref));
    ...
    foreach ($editor_field_names as $editor_key) {
         if (($fields[$editor_key] != null) && (array_search($fields[$editor_key],$editors)) === false) {
              ...
              return "Error: unknown editor in $editor_key";
         }
    }
...
function count_drafts($draftname) {
     ...
     $db_link = open_db();
     if ($result = mysqli_query($db_link,$sql_statement)) {
          $count = mysqli_fetch_array($result);
          close_db();
     } else {
          rfc_fail("Database error in count_drafts: " . mysqli_error($db_link));
     }
     if ($debug_edit_lib === true) {
          print("Result of count query for  $draftname is ");
          var_dump($count);
          print("</p>");
     }
     return $count['count(draft)'];
}
...
function is_duplicate_draftname($editing_key, $draftname) {
     ...
     if ($result = mysqli_query($db_link,$sql_statement)) {
          ...
          if ($row_count ==  0) { // no drafts with this name
               close_db();        // close database connection and return
               return false;
          }
          $row = mysqli_fetch_array($result);
     } else {
          rfc_fail("Database error in duplicate_draftname_check: " . mysqli_error($db_link));
     }
...
function state_adjust_logic(){
   ...
   /*   has come from the database with 'N' and need to show reference chain with 'Y' */
    ...
    print "For draft : ". $fields['draft']."&nbsp;&nbsp;&nbsp;&nbsp;Temporary state : " .$old_state." &nbsp;&nbsp;&nbsp;&nbsp;Final state : ". $new_state;                                                            
      
    if ($new_state != $old_state){   
#	   print "For draft :". $fields['draft'] ." Old state : " .$old_state ." New state : ". $new_state;
           $db_link = open_db();
           $index_status = update_index($fields['internal_key'],$out_state_id,$out_iana_flag,$out_ref_flag,$out_generation_number,$db_link);
      	   if ($index_status){ //insert state data to state history table 
           } else {
           rfc_fail("Update failed on Index: state_adjust_logic for $draft" . mysqli_error($db_link));
       	   }
    } 
	   ...
	   if ($out_generation_number > '0'){
           ...
           if ($new_state != $old_state){   
   	           print "<tr><td>$draft modified from state : $old_state -> $new_state</td></tr>";                                                            	                $version_number = substr($draft,-2); //Get the version number	
                   $db_link = open_db();
          	   $status = insert_to_state_history($internal_key,$out_state_id,$out_iana_flag,$out_ref_flag,$out_generation_number,$version_number,$db_link);
          	   if ($status){ //insert state data to state history table
   	           } else {
               	    rfc_fail("Insert failed on State History: run_check_on_missref for $draft" . mysqli_error($db_link));
       	           }
                   $index_status = update_index($internal_key,$out_state_id,$out_iana_flag,$out_ref_flag,$out_generation_number,$db_link);
          	   if ($index_status){ //insert state data to state history table 
   	           } else {
               	    rfc_fail("Update failed on Index: run_check_on_missref for $draft" . mysqli_error($db_link));
       	           }
	   ...
	   if ($out_generation_number > '0'){
           ...
           if ($new_state != $old_state){   
          	   ...
          	   if ($status){ //insert state data to state history table
               	    ...
               	    rfc_fail("Insert failed on State History: run_check_on_other for $draft" . mysqli_error($db_link));
       	           }
                   $index_status = update_index($internal_key,$out_state_id,$out_iana_flag,$out_ref_flag,$out_generation_number,$db_link);
          	   if ($index_status){ //insert state data to state history table 
   	           } else {
               	    rfc_fail("Update failed on Index: run_check_on_other for $draft" . mysqli_error($db_link));
       	           }
...
function get_generation_draft($generation_number){
   ...
   if ( ($result = mysqli_query($db_link,$query))) {
 	 while ( $line = mysqli_fetch_assoc($result)){ 
		 ...
		 rfc_fail("get_generation_draft :Query : $query " . mysqli_error($db_link));
     }
    close_db(); 
...
function get_other_draft(){
   ...
   $query = "SELECT `internal_key`,`DRAFT`, `REF`, `ref_flag`, `iana_flag`, `state_id` from `index` where `state_id` NOT IN ('3','14','16','19','22') AND `ref` NOT IN ('','NULL','+')"; 
   ...
   if ( ($result = mysqli_query($db_link,$query))) {
 	 while ( $line = mysqli_fetch_assoc($result)){ 
		 ...
		 rfc_fail("get_other_draft :Query : $query " . mysqli_error($db_link));
     }
    close_db(); 
...
/******************************************************************************************************/
function insert_to_state_history($internal_key,$state_id,$iana_flag,$ref_flag,$generation_number,$version_number,$con) {
...
END;
    ...
    if (($err = open_db() ) ) rfc_fail("Open DB: " . mysqli_error($err));
...
/*NOTE*/
	...
	if ( $field_mysqli_cols[$key] && $value) {
	    ...
	    if ($values == "(" ) 
		$values .= '"' .mysqli_real_escape_string($err,$value). '"';
	    else 
		$values .= ', "' .mysqli_real_escape_string($err,$value). '"';
	}
    }
    ...
    if (mysqli_query($err,$insert)) {
         $fields['internal_key'] = get_new_internal_key($fields['draft']);
         state_adjust_logic();
	 $version_number = substr($fields['draft'],-2); //Get the version number	
  	 $status = insert_to_state_history($fields['internal_key'],$fields['state_id'],$fields['iana'],$fields['ref_flag'],$fields['generation_number'],$version_number,$err);
       	 if ($status){ //insert state data to state history table 
 	 } else {
               	 rfc_fail("Insert failed on State History: Add draft" . mysqli_error($err));
       	 }
	    ...
	    if ($post_status == 'OK'){
		  ...
		  print "$post_status";
            ...
            print <<<END
<p>
    Draft <span class="draftname">$fields[draft]</span> has been added to the 
...
END;
                ...
                send_new_add_msg_form($fields);
                 ...
                 rfc_fail("Insert failed on editor assignment: " . mysqli_error($err));
            } 

    } else { rfc_fail("Insert failed: " . mysqli_error($err)); }
    close_db();
}
...
function get_new_internal_key($draft) {
     ...
     if (($result = mysqli_query($db_link,$query))) {
          ...
          $internal_key = -1;
          error_log("get_new_internal_key: database error: " . mysqli_error($db_link) . ": $query");
     }
     close_db();
     return $internal_key;
}
...
#   	    adjust_state(); // Uses DB to change $fields[state] BEFORE building update MISSREF calculation
/*NOTE*/
/*Check referecnes of the newly added draft to get the it exact state  */
/*Insert this state to index table but do not store it in state history*/
/*This state might change during missref draft state adjustment logic  */
...
#                  if ($key == 'state') $chg_state = true; //Check if the state is change for the draft
                   if ($template) { // data is different so add
                        $template .= ", " . $field_mysqli_cols[$key] . " = " .
                             ($fields[$key] == null ? " NULL " :
                              '"' .mysqli_real_escape_string($err,$fields[$key]). '"');
                   } else {
                        $template .= "set " . $field_mysqli_cols[$key] . " = " . 
                             ($fields[$key] == null ? " NULL " :
                              '"' .mysqli_real_escape_string($err,$fields[$key]).'"');
                   }
              }
              ...
              if ($o_value != null && strlen(trim($o_value)) > 0) {
                   if ($template) {
                        $template .= ", " . $field_mysqli_cols[$key] . " = NULL";
                   } else {
                        $template .= "set " .  $field_mysqli_cols[$key] . " = NULL";
                   }
              }
...
END;
    ...
    if ( $template ) {
         $update = "update `index` $template where " . 
              $field_mysqli_cols[$fields['keyfield']] . " = ";
         if ($fields['keyfield'] == 'internal_key') {
              $update .= $fields[$fields['keyfield']];
         } else {
              $update .= "\"" . $fields[$fields['keyfield']] . "\"";
         }
         ...
         if ($chg_cid == true) {
              require_once('cluster_support_lib.php');
              if (modify_cluster($fields,$old_fields)) {
                   print<<<END
...
/*                         print<<<END */
/* <h2>Did not update cluster data.</h2> */

/* END; */
              }
         }

         if (mysqli_query($err,$update)) {
	  	...
	  	$status = insert_to_state_history($fields['internal_key'],$fields['state_id'],$fields['iana'],$fields['ref_flag'],$fields['generation_number'],$version_number,$err);
        	 if ($status){ //insert state data to state history table 
                	 ...
                	 rfc_fail("Insert failed on State History: " . mysqli_error($err));
           	   }
              }
...
END;
              ...
              rfc_fail("Commit failed: " . mysqli_error($err));
...
#function get_stream_name($doc_id) {
...
function get_state_only(){
     ...
     if ($result = mysqli_query($db_link,$query)){
	     while ($row_state_id = mysqli_fetch_assoc($result)){
           ...
           rfc_fail("Database error in get_state_only:" . mysqli_error($db_link));
     }
...
END;
        ...
        $db_link = open_db();
        if ($result = mysqli_query($db_link,$query)){
	        while ($row_state = mysqli_fetch_assoc($result)){
	        ...
	        print "</tr>";
	          ...
	          rfc_fail("Database error in get_edited_state:" . mysqli_error($db_link));
	}
        close_db();         
}
...
function get_state_id($getfields){
     ...
     if (is_array($getfields)){	
	     $str_getfields = implode("', '", $getfields);
     }else {
	     $str_getfields = $getfields;		  	
     }
		
     $selected_state_id = array();
     $query_selectlookup = "SELECT state_id FROM `states` where STATE_NAME IN";
     $query_selectlookup .= "("."'".$str_getfields."'".")";
 

     $db_link = open_db();
     $result_state_id = mysqli_query($db_link,$query_selectlookup);
     while ($row_state_id = mysqli_fetch_assoc($result_state_id)){
            array_push($selected_state_id,$row_state_id['state_id']);
     }
...
function check_draft_status($in_draft) {
 ...
 $query = "select internal_key, DRAFT,state_id, ref, ref_flag, iana_flag ,generation_number from `index` 
	   where 
		draft >=\"$in_draft-00\" and draft <= \"$in_draft-99\" and
		state_id not in(\"3\",\"19\") and
...
#             array_push($draftstatus,$line);
	  ...
	  print "Error". mysqli_error($db_link);
      }
 mysqli_free_result($result);
 close_db();
...
function update_index($internal_key,$out_state_id,$out_iana_flag,$out_ref_flag,$out_generation_number){
   ...
   if (mysqli_query($db_link,$query)) {
	...
	$flip_status = 0;
         rfc_fail("Update failed for flipping state update_index: " . mysqli_error($db_link));
