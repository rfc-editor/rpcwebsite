...
# $Id: rfc_state_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: rfc_state_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
# $Id: rfc_state_lib.php,v 1.2 2017/01/11 20:53:08 priyanka Exp $
...
function retrieve_author_emails($internal_key) {
     ...
     $result = mysqli_query($db_link,$query) or die ("Query \"" . $query . "\" failed : " . mysqli_error($db_link));
     $addresses = mysqli_result($result, 1);
     close_db();
     if ($addresses === false) {
...
function retrieve_working_group_emails($source) {
     ...
     $wgResult = mysqli_query($db_link,$wgQuery) or die ("Query failed " . mysqli_error($db_link));

     $wgLine = mysqli_fetch_assoc($wgResult);
     close_db();
     if ($wgLine === false) {
          print("<p class=\"error\">Failed to retreive working_group data. SQL query was \"" . htmlspecialchars($wgQuery) . "\".</p>");
          $rsl_error = true;
     }

     if (!$rsl_error) {
          $wgname = $wgLine['wg_acronym'];
          $wg_addrs = $wgname."-ads@tools.ietf.org, " . $wgname."-chairs@tools.ietf.org";
...
function get_wg_ssp_id($draftname) {
     ...
     $wg_ssp_id = mysqli_fetch_assoc($result);
     if ($debug_rsl === true) {
