...
#$run_month = date("m", strtotime("-2 month") ) ;
$run_month_full = date("F", strtotime("-1 month") ) ;
#$run_month_full = date("F", strtotime("-2 month") ) ;

$rfced_time_html=fopen("/a/www/staff/rfced_report/rfced_time$run_month$run_year.html",'w');
...
function get_body(){
 ...
 $body_text = "<h2 class=\"simple_center\">RFC Editor Time - $run_month_full $run_year</h2>\n";
...
$rfced_query = "SELECT i.`DOC-ID`, i.`internal_key`, i.`DRAFT`, i.`state_id`, i.`PUB-date` from `index` i where  i.state_id = :state_id and i.`pub-date` LIKE :pub_date and i.`DOC-ID` LIKE :rfc ORDER BY `DOC-ID`";

try {
   $stmt = $pdo->prepare($rfced_query);
   $stmt->bindValue('state_id',14);
   $pub_date = $run_year.'-'.$run_month.'%';
   ...
   $num_of_rows = $stmt->rowCount();
       ...
       error_log("Error processing : get_rfced_data rfced_report", $pe->getMessage(), $pe->getCode(), array('exception' => $pe));
}
...
$temp_rfced_query = "CREATE TABLE temp_rfced_data (
doc_id varchar(10),
draft varchar(200),
...
try {
   ...
   if ($debug_monchg === true) {
       ...
       error_log("Error processing : temp_rfced_cur_data rfc_ed_time", $pe->getMessage(), $pe->getCode(), array('exception' => $pe));
}
...
for ($row = 0 ; $row < $rfced_count; $row++){
   ...
   try {        
       ...
       $rfced_num_of_rows = $data_stmt->rowCount();
         ...
         error_log("Error processing : rfced_data_query rfced_time", $pe->getMessage(), $pe->getCode(), array('exception' => $pe));
   }
   ...
   $insert_query = "INSERT INTO `temp_rfced_data` (doc_id,draft,wks_in_edit,wks_in_editor,rfc_ed_time) VALUES (:doc_id,:draft_name,:edit_wks_in_state,:editor_wks_in_state,:rfc_ed_time)";
...
$rfced_sort_query = "SELECT `doc_id`, `draft`, `wks_in_edit`, `wks_in_editor`, `rfc_ed_time` from `temp_rfced_data` ORDER BY `rfc_ed_time`";
try {
   $stmt = $pdo->query($rfced_sort_query);
   $num_of_rows = $stmt->rowCount();
} catch (PDOException $pe){
       error_log("Error processing : get_sorted_data rfced_time", $pe->getMessage(), $pe->getCode(), array('exception' => $pe));
}
...
if ($num_of_sort > 0) {
  ...
  }#End of while  while($rfced_data_line = mysqli_fetch_row($rfced_data_result)){ 
 }#End of if ($num_of_rows > 0) {
...
$drop_query = "DROP TABLE `temp_rfced_data`";
try {
   $pdo->exec($drop_query);
   if ($debug_monchg === true) {
